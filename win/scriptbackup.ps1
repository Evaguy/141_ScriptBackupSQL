...
# backup name + dbs backup config inside of an infinite loop if the user doesn't have a psw
while ($true) {
...
# backup name + dbs backup config inside of an infinite loop if the user has a psw
# while ($true) {
    # $BackupFileName = "db_dump_" + (Get-Date -Format "dd_MM_yyyy_HH_mm_ss") + ".sql" 
