...
# backup name + dbs backup config inside of a infinite loop if the user doesn't have a password
while ($true) {
...
# backup name + dbs backup config inside of a infinite loop if the user has a password
# while ($true) {
    # $BackupFileName = "db_dump_" + (Get-Date -Format "dd_MM_yyyy_HH_mm_ss") + ".sql" 
